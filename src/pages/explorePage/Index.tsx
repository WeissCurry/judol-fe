...
export default function ExplorePage() {
  ...
  // NEW STATE: Quick Filter (Sinta 1 / Top Rated)
    ...
    if (localData) {
      ...
      const formattedLocalData = parsedData.map((item: any) => {
        ...
        return {
          ...
          authorOrg: item.author?.org || "UIN Syarif Hidayatullah",
          ...
          sinta: sintaRank,
          aiScore: scoreVal || 0,
          tierLabel: item.tier || "Unverified",
  ...
  const filteredPapers = papers.filter((paper) => {
    ...
    if (quickFilter === 'sinta1') {
      matchesQuickFilter = paper.sinta === "1" || paper.sinta === 1;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterType, quickFilter]); // Add quickFilter dependency
